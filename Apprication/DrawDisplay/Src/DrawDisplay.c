...
static void Clear_StringSpace(uint16_t x,uint16_t y,char *str,uint16_t size)
{
...
void Dispaly_DigitalClock(RTC_TimeTypeDef *time,RTC_DateTypeDef *date)
{
  ...
  if((++ProcessCount_Clock)>PROCESSTIME_CLOCK)
  {
    ...
    Clear_StringSpace(0,120,ClockString,9);
  ...
  if((++ProcessCount_Envir)>PROCESSTIME_ENVIR)
  {
    ...
    Clear_StringSpace(0,0,EnvirString[0],5);
    ILI9325_DrawString(0,0,EnvirString[0],ILI9325_BLUE,5);

    sprintf(EnvirString[1],"%2f",SHT31_Get_Humidity());
    Clear_StringSpace(160,0,EnvirString[1],5);
    ILI9325_DrawString(160,0,EnvirString[1],ILI9325_BLUE,5);
